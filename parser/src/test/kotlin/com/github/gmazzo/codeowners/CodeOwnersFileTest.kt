...
class CodeOwnersFileTest {
    ...
    fun `should handle paths with spaces`() {
        testParse("path\\\\ with\\\\ spaces/ @space-owner", CodeOwnersFile(
            CodeOwnersFile.Entry("path\\\\ with\\\\ spaces/", "@space-owner"),
        ))
    }

    @Test
    fun `should handle comments`() {
        testParse(" # This is a comment ", CodeOwnersFile(
    ...
    fun `should handle inline comments`() {
        testParse("/foo/bar    @foo @bar   #  This is an inline comment. ", CodeOwnersFile(
    ...
    fun `should handle paths slash# sequences`() {
        testParse("\\\\#foo @foo-owner", CodeOwnersFile(
            CodeOwnersFile.Entry("\\\\#foo", "@foo-owner"),
        ))
    }
